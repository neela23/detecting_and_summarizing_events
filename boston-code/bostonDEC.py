...
def createGraph(text, date):
	...
	edges[edgeIndex].extend(listOfEdges)
...
#G = nx.read_weighted_edgelist('/scratch2/navudai/boston/graph/graph181weighted.edgelist')
#print len(G)
...
for i in range(1,217):
	fileName = "/scratch2/navudai/bostonfiles/file"+`i`+".csv"
	tweetFiles.append(fileName)


for i in range(0, 216):#range(len(tweetFiles)):
	decreaseEdgeWeights()
	getTextFromJson(tweetFiles[i], i+1)
	...
	if(len(G)==0):
		continue
	ecentrality = calculateEigenvector(G)	
	...
	for node in ecentrality:
		cQueue = centralityQueue.setdefault(node, deque())	
		
		if len(cQueue)>=5:
			cQueue.popleft()
		cQueue.append(ecentrality[node])
	...
	for node in centralityQueue:
		...
		#print (node, ecentrality[node],x,y,slope)
		ecentrality[node]*=slope
	'''	
	numberOfElements = len(G) if len(G)<100 else 100;
	
	etarget = heapq.nlargest(numberOfElements, ecentrality, key = ecentrality.get)                       # Pick elements with largest centrality.
	#ptarget = htarget = heapq.nlargest(numberOfElements, pr, key = pr.get)
 	'''

	fileName = "/scratch2/navudai/boston/dynamicECall/ecentrality"+`i+1`+".txt"
	...
	for node in ecentrality:
		ecentral.write(node)
		ecentral.write(" ")
		ecentral.write(str(ecentrality[node]))
		#ecentral.write(" ")
                #ecentral.write(str(frequencyDict[node]))
        	ecentral.write('\n')

	if(i%50 is 0):
		gloc = "/scratch2/navudai/boston/graph/graph"+`i+1`+".weighted.edgelist";
