...
for i in range(0, 216):#range(len(tweetFiles)):
	...
	deg1 = G.degree(weight='weight')
	deg = dict((k,float(v)) for k,v in deg1.iteritems())

	fileName = "/scratch2/navudai/boston/dynamicDegree/deg"+`i+1`+".txt"
	...
	'''
        for node in deg:
                ecentral.write(node)
                ecentral.write(" ")
                ecentral.write(str(deg[node]))
                #ecentral.write(" ")
                #ecentral.write(str(frequencyDict[node]))
                ecentral.write('\n')
	'''
	
	maxKey = max(deg , key = deg.get)
        maxC = deg[maxKey]
        print maxKey, maxC
        for key in deg:
                deg[key] = deg[key]/maxC

	
	'''
	deg = calculateEigenvector(G)	
	
	maxKey = max(deg , key = ecentrality.get)
	maxC = deg[maxKey]
	print maxKey, maxC
	for key in deg:
		deg[key] = ecentrality[key]/maxC
	...
	etarget = heapq.nlargest(numberOfElements, deg, key = ecentrality.get)                       # Pick elements with largest centrality.
	#ptarget = htarget = heapq.nlargest(numberOfElements, pr, key = pr.get)
 	

	fileName = "/scratch2/navudai/baltimore/ECall/deg"+`i+1`+".txt"
